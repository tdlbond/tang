<template>
  <div class="home-main">
    <div
      ref="d1Ref"
      class="div d1"
      :class="[transformClass.d1Class]"
      @mouseenter="(e) => handleMouseenter(e, 1)"
    >
      <div
        v-if="current == 1"
        class="show"
      >
        ...
        <div class="content">
          ...
          <div>
            多个来自不同作者的 mixin
            可能会注册相同的属性名，造成命名冲突。若使用组合式函数，你可以通过在解构变量时对变量进行重命名来避免相同的键名。多个
            mixin
      ...
      >
        ...
        <div>
          组合式函数在<code>script setup</code
          >或<code>setup()</code>钩子中，应始终被同步地调用。在某些场景下，你也可以在像
          onMounted() 这样的生命周期钩子中使用他们。
        </div>
      </div>
    </div>
    <div
      ref="d2Ref"
      class="div d2"
      :class="[transformClass.d2Class]"
      @mouseenter="(e) => handleMouseenter(e, 2)"
    >
      <div
        v-if="current == 2"
        class="show"
    ...
    <div
      ref="d3Ref"
      class="div d3"
      :class="[transformClass.d3Class]"
      @mouseenter="(e) => handleMouseenter(e, 3)"
    >
      <div
        v-if="current == 3"
        class="show"
...
const handleMouseenter = (e: any, index: number): void => {
  const clientHeight = document.body.clientHeight
  const rect = e.target.getBoundingClientRect()
  if (index == current.value) return
  const front = index + 1 > 3 ? 1 : index + 1
  const back = index - 1 < 1 ? 3 : index - 1
  if (rect.left != 0 && rect.height != clientHeight) {
    transformClass[`d${index}Class` as 'd1Class' | 'd2Class' | 'd3Class'] =
      'transd1'
    transformClass[`d${front}Class` as 'd1Class' | 'd2Class' | 'd3Class'] =
      'transd2'
    transformClass[`d${back}Class` as 'd1Class' | 'd2Class' | 'd3Class'] =
      'transd3'
  }
  if (
    (rect.x != 0 && rect.y == 0 && rect.height != clientHeight) ||
    (rect.x != 0 && rect.y != 0 && rect.height != clientHeight)
  ) {
    transformClass[`d${index}Class` as 'd1Class' | 'd2Class' | 'd3Class'] =
      'transd4'
